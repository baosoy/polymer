...
const useStore = create<PolymerStore>((set, get) => ({
  ticks: 0,
  environment: {
    values: {
      pm10: 0,
    ...
    if (funds >= args.funds) {
      set((state) => ({
        ...
        plasticWastePerMeter: args.plasticWastePerMeter
          ? Math.round(state.plasticWastePerMeter * args.plasticWastePerMeter)
  ...
  generateAutomatedRecycledPlastic: () => {
    ...
    if (plasticWaste >= metersNeeded) {
      set((state) => ({
        plasticWaste:
          state.plasticWaste - state.plasticWastePerMeter * state.autorecyclers,
